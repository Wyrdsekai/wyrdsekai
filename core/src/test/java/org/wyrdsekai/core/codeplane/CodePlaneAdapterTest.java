...
class CodePlaneAdapterTest {

    private CodePlaneAdapter adapter;

    @BeforeEach void setUp() {
        adapter = new CodePlaneAdapter();
    }
    ...
    @Test void translateToRoom_creates_bridge_event() {
        ...
        assertThat(event.get().source()).isEqualTo("codeplane");
        assertThat(event.get().targetId()).isEqualTo("the-forge");
    }

    @Test void translateToCodePlane_creates_bridge_event() {
        adapter.mapBoard("board-1", "the-forge", "Dev Board", Map.of(), Map.of());
        var event = adapter.translateToCodePlane("speech", "the-forge",
    ...
    @Test void describe_shows_summary() {
        ...
        assertThat(desc).contains("CodePlane Integration");
