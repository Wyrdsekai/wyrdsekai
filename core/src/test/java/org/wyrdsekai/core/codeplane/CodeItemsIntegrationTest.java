...
import org.wyrdsekai.core.coding.CodePlaneEventAdapter;
 ...
 *
 * <p>Post-Phase-2 cleanup: the legacy {@code CodePlaneItemBridge} +
 * {@code CodexItem} / {@code ArtifactItem} shims are gone. The full
 * pipeline now runs through the generic {@link CodingTaskItemBridge} +
 * {@link CodePlaneEventAdapter} pair against {@link CodeItemStore} and
...
class CodeItemsIntegrationTest {
    ...
    private AgentEvent.ZoneBroadcast boardCompletionEvent(
        ...
        var zoneResponse = new S2CMessage.ZoneResponse(
            1L, "req-1", CodePlaneBackend.NAME, "Board completed", data, List.of());
    ...
    @Test void bridge_ignores_non_completion_events() {
        ...
        // ZoneBroadcast with codeplane namespace but a status event (not board_completed)
        ...
        var statusEvent = new AgentEvent.ZoneBroadcast(
            CodePlaneBackend.NAME, "workshop",
            new S2CMessage.ZoneResponse(2L, "req-2", CodePlaneBackend.NAME,
                "Board running", statusData, List.of()),
            Instant.now());
        bridge.accept(statusEvent);

        // ZoneBroadcast with codeplane namespace but Prose message (not ZoneResponse)
        ...
        // ZoneBroadcast with codeplane ZoneResponse but null data
        var nullDataEvent = new AgentEvent.ZoneBroadcast(
            CodePlaneBackend.NAME, "workshop",
            new S2CMessage.ZoneResponse(4L, "req-3", CodePlaneBackend.NAME,
                "No data", null, List.of()),
            Instant.now());
        bridge.accept(nullDataEvent);

        // ZoneBroadcast with codeplane ZoneResponse but no "event" field
        ...
        var noEventFieldEvent = new AgentEvent.ZoneBroadcast(
            CodePlaneBackend.NAME, "workshop",
            new S2CMessage.ZoneResponse(5L, "req-4", CodePlaneBackend.NAME,
    ...
    @Test void room_object_placement_callback_fires() {
        ...
        customRegistry.register(new CodePlaneEventAdapter(generator));
