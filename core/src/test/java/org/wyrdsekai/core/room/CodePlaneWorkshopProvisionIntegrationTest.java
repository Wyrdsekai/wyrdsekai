 ...
 * ZoneGuardian.ProvisionCodePlaneWorkshop} through the real actor mailbox.
 *
 ...
 */
@Tag("integration")
class CodePlaneWorkshopProvisionIntegrationTest {
    ...
    @Test void provisionCodePlaneWorkshop_spawns_room_actor() throws InterruptedException {
        ...
        guardian.tell(new ZoneGuardian.ProvisionCodePlaneWorkshop("u-operator", "Masumi"));
        ...
        assertThat(snap.aliases())
            .contains("workshop", "codeplane", "code", "studio");
    ...
    @Test void provisionCodePlaneWorkshop_idempotent() throws InterruptedException {
        var guardian = testKit.spawn(
            ZoneGuardian.create(null, List.of(), null, null));
        Thread.sleep(3500);
        ...
        guardian.tell(new ZoneGuardian.ProvisionCodePlaneWorkshop("u-idem", "Idem"));
        Thread.sleep(500);
        guardian.tell(new ZoneGuardian.ProvisionCodePlaneWorkshop("u-idem", "Idem"));
        Thread.sleep(500);
        ...
        roomRef.tell(new RoomCommand.GetSnapshot(probe.getRef()));
        var snap = probe.receiveMessage(Duration.ofSeconds(5));
        assertThat(snap.name()).isEqualTo("Idem's CodePlane Workshop");
    }

    @Test void provisionCodePlaneWorkshop_multi_bondholder_distinct_rooms()
        ...
        guardian.tell(new ZoneGuardian.ProvisionCodePlaneWorkshop("user-a", "Alice"));
        guardian.tell(new ZoneGuardian.ProvisionCodePlaneWorkshop("user-b", "Bob"));
        Thread.sleep(800);
        ...
        assertThat(probe.receiveMessage(Duration.ofSeconds(5)).name())
            .isEqualTo("Alice's CodePlane Workshop");
        roomB.tell(new RoomCommand.GetSnapshot(probe.getRef()));
        assertThat(probe.receiveMessage(Duration.ofSeconds(5)).name())
            .isEqualTo("Bob's CodePlane Workshop");
    }
}
