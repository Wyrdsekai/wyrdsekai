...
# max_tokens raised 700 -> 1800 (2026-08-05). 700 was sized for an ANSWER,
# but reasoning models spend it on deliberation and get cut off before they
# speak: our own 9B produced reasoning-only (no answer) on 3 of 11 probes.
# The budget has to cover thinking AND the reply, or the probe measures our
# ceiling instead of the substrate.
def chat(endpoint, system, user, max_tokens=1800, temperature=0.7):
    ...
    # HARNESS BUG FOUND 2026-08-05, after the full-roster run.
    # Several substrates returned an EMPTY content field while the response
    # still reported a tok/s figure — i.e. tokens WERE generated and we threw
    # them away. llama.cpp splits deliberation into `reasoning_content`, and
    # `--reasoning-budget 0` does not stop every model from emitting it. Reading
    # only `content` silently discarded whole replies, and it did so worst on
    # the open-register probes (P7-P10) where deliberation is the entire point
    # — so the corpus is missing exactly the answers we most wanted.
    # Keep both, and label the reasoning so a reader can tell them apart.
...
def main():
    ...
    outdir = pathlib.Path("/opt/campaign-local/encounters")
