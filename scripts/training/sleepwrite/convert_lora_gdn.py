    ...
    with torch.inference_mode():
        ...
        class LoraModel(model_class):  # ty: ignore[unsupported-base]
            ...
            def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
                # ── wyrdsekai GDN patch (2026-08-29) ─────────────────────────
                # The qwen3.5 base conversion permutes linear_attn.out_proj
                # COLUMNS (v-head reorder). A pure column permutation rides on
                # the factors exactly: W' = W[:, perm]  =>  A' = A[:, perm],
                # B unchanged. The framework path instead tries to reshape the
                # factor pair and raises NotImplementedError, so handle these
                # tensors here and skip it. Any OTHER linear_attn target is
                # genuinely not factor-expressible (fused/row-reordered) —
                # refuse loudly rather than convert wrongly.
                if ".linear_attn." in name and isinstance(data_torch, LoraTorchTensor):
                    if ".out_proj." not in name:
                        raise ValueError(
                            f"LoRA on {name} is not convertible: the base "
                            "conversion fuses/row-reorders this tensor. Only "
                            "linear_attn.out_proj adapters are supported.")
                    nk = self.hparams.get("linear_num_key_heads", 0)
                    nv = self.hparams.get("linear_num_value_heads", 0)
                    lora_a, lora_b = data_torch.get_lora_A_B()
                    if nk and nv and nk != nv:
                        hd = self.hparams["linear_value_head_dim"]
                        col_perm = self._reorder_v_heads(
                            torch.arange(nv * hd, dtype=torch.long).unsqueeze(0),
                            1, nk, nv // nk, hd).squeeze(0)
                        lora_a = lora_a[:, col_perm]
                    dest_name = self.map_tensor_name(name)
                    yield (dest_name + ".lora_a", lora_a)
                    yield (dest_name + ".lora_b", lora_b)
                    return
                # ── end wyrdsekai GDN patch ──────────────────────────────────
