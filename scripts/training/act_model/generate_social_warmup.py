...
TOOLS = {
    ...
    "sending_stone":  fn("sending_stone", "Reach a companion present with a sending — your in-room reach toward them.", {"target": SS(), "message": SS()}, ["target", "message"]),
...
BEHAVIORS = [
    ...
    ("reach", "affiliation", "sending_stone", None, ["introspect_bondholder_floor", "examine"],
    ...
    ("care", "care", "sending_stone", None, ["introspect", "acknowledge_harm"],
...
def act_args(behavior, lang):
    ...
    if mtool in ("tell_agent", "sending_stone"):
...
def main():
    ...
    ap.add_argument("--out-dir", default="scripts/training/act_model")
    ...
    for lang in LANGS:
        for b in BEHAVIORS:
            n_act = args.n_per_behavior * (5 if b[0] in ("reach","care") else 1)
