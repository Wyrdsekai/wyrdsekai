...
if [[ "${WYRDSEKAI_SKIP_MODELS:-0}" != "1" ]]; then
    if [[ "$TIER" == "gpu" ]]; then
        run_svc llama-drive /app/llama-server --model "$M9B" --host 0.0.0.0 --port 8200 \
            --ctx-size $(( 16384 * LLAMA_PARALLEL )) --jinja --reasoning off --reasoning-budget 0 --cache-ram "${LLAMA_CACHE_RAM:-1024}" --flash-attn on -ngl 99 --parallel "$LLAMA_PARALLEL"
        ...
        run_svc llama-voice /app/llama-server --model "$M4B" --host 0.0.0.0 --port 8201 \
            --ctx-size $(( 16384 * LLAMA_PARALLEL )) --jinja --reasoning off --reasoning-budget 0 --cache-ram "${LLAMA_CACHE_RAM:-1024}" -ngl 99 --parallel "$LLAMA_PARALLEL"
    else
        # CPU: one 4B serving as the single backend
        run_svc llama-drive /app/llama-server --model "$M4B" --host 0.0.0.0 --port 8200 \
            --ctx-size $(( 8192 * LLAMA_PARALLEL )) --jinja --reasoning off --reasoning-budget 0 --cache-ram "${LLAMA_CACHE_RAM:-1024}" --parallel "$LLAMA_PARALLEL"
    fi
fi
